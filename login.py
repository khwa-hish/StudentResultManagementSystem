...
class Login:
    def __init__(self, home):
        self.home=home
        self.home.title("Student Result Login")
        self.home.geometry("1400x700+0+0")
        #====BG IMage=====
        # self.bgImage=Image.open("Images/7.jpg")
        # self.bgImage=self.bgImage.resize((1510,850),Image.Resampling.LANCZOS)
        # self.bgImage=ImageTk.PhotoImage(self.bgImage)

        # self.lbl_bg=Label(self.home,image=self.bgImage).place(x=10,y=150,width=1510,height=850)

        self.bgImage=Image.open("Images/6.jpg")
        self.bgImage=self.bgImage.resize((1540,850),Image.Resampling.LANCZOS)
        self.bgImage=ImageTk.PhotoImage(self.bgImage)
        self.lbl_bg=Label(self.home,image=self.bgImage).place(x=0,y=0,width=1540,height=850)

        Frame_login=Frame(self.home,bg="white")
        Frame_login.place(x=500,y=100,height=550,width=500)

        # Login title
        title=Label(Frame_login,text="User Login",font=("Times New Roman",25,"bold"),fg="black",bg="white").place(x=180,y=20)
        
        #Username and password lable
        lbl_user=Label(Frame_login,text="User Name",font=("Times New Roman",15,"bold"),fg="black",bg="white").place(x=45,y=140)
        self.txt_user=Entry(Frame_login,font=("times new roman",15),bg="white")
        self.txt_user.place(x=45,y=170,width=400,height=35)


        lbl_pass=Label(Frame_login,text="Password",font=("Times New Roman",15,"bold"),fg="black",bg="white").place(x=45,y=240)
        self.txt_pass=Entry(Frame_login,font=("times new roman",15),bg="white")
        self.txt_pass.place(x=45,y=270,width=400,height=35)

        forget=Button(Frame_login,text="Forget Password?",cursor="hand2",command=self.forget_window,bg="white",fg="darkgreen",font=("times new roman",15,"bold")).place(x=320,y=320)
        
        #Buttons
        signup=Button(Frame_login,text="Sign Up",cursor="hand2",command=self.register_window,bg="green",fg="white",font=("times new roman",12,"bold")).place(x=230,y=490,height=40)

        login=Button(Frame_login,command=self.login_function,text="Login",cursor="hand2",bg="indigo",fg="white",font=("times new roman",20,"bold")).place(x=210,y=370,width=100,height=50)

        lbl_create=Label(Frame_login,text="Didn't have an account?",font=("Times New Roman",15,"bold"),fg="darkgreen",bg="white").place(x=170,y=450)

        #Checkbuttons
        UserType=Label(Frame_login,text="User : ", font=("times new roman", 15, "bold"), bg="lightyellow",fg="black").place(x=45,y=85)
        ...
        if self.txt_user.get()=="":
            ...
            try:
                ...
                if row==None:
                    ...
                    self.home2.config(bg="lightgreen")
                    self.home2.focus_force()
                    self.home2.grab_set()
                #Title for Forget Interface
                    title=Label(self.home2,text="Forget Password",font=("Times New Roman",25),fg="white",bg="black").place(x=0,relwidth=1)
                #Contents in Forget Password Page
                    question=Label(self.home2, text="Security Question", font=("times new roman", 15, "bold"), bg="lightgreen",fg="black").place(x=650,y=150)
                    self.cmb_quest=ttk.Combobox (self.home2, font=("times new roman",13), state="readonly",justify=CENTER) 
                    self.cmb_quest['values']=("Select", "Your First Pet Name", "Your Birth Place", "Your Best Friend Name") 
                    self.cmb_quest.place(x=650, y=180,width=250)
                    self.cmb_quest.current (0)
        
                    answer=Label(self.home2, text="Answer", font=("times new roman",15,"bold"),bg="lightgreen", fg="black").place(x=650,y=230)
                    self.txt_answer=Entry(self.home2, font=("times new roman", 15),bg="white")
                    self.txt_answer.place(x=650, y=260,width=250)

                    new_passwd=Label(self.home2, text="New Password", font=("times new roman",15,"bold"),bg="lightgreen", fg="black").place(x=650,y=310)
                    self.txt_new_passwd=Entry(self.home2, font=("times new roman", 15),bg="white")
                    self.txt_new_passwd.place(x=650, y=340,width=250)

                    btn_change_passwd=Button(self.home2,text="Reset Password",command=self.forget_passwd,bg="black",fg="White",font=("times new roman",15,"bold")).place(x=700,y=450)
